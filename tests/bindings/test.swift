...
do  {
    ...
    let uuid = UUID.init()
    try await vdc.add(credential: Credential(id: uuid.uuidString, format: CredentialFormat.msoMdoc, type: CredentialType("org.iso.18013.5.1.mDL"), payload: mdocBytes!, keyAlias: "alias"))
    let mdlSession = try await initializeMdlPresentation(uuid: uuid.uuidString, storageManager: storage)
    assert(try! mdlSession.terminateSession() == Data([0xa1, 0x66, 0x73, 0x74, 0x61, 0x74, 0x75, 0x73, 0x14]))
} catch {
    assert(false, "\(error)")
}
